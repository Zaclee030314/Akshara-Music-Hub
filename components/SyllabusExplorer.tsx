    ...
    music: [
        {
            ...
            subjects: [
                ...
                { id: Subject.PIANO, label: 'Piano / Keyboard', icon: '🎹', color: 'bg-sky-100 text-sky-700' },
                { id: Subject.DRUMS, label: 'Drums', icon: '🥁', color: 'bg-red-100 text-red-700' },
                { id: Subject.WESTERN_VOCAL, label: 'Voice / Singing', icon: '🎤', color: 'bg-pink-100 text-pink-700' },
                { id: Subject.VIOLIN, label: 'Violin', icon: '🎻', color: 'bg-violet-100 text-violet-700' },
                { id: Subject.TRUMPET, label: 'Trumpet', icon: '🎺', color: 'bg-yellow-100 text-yellow-700' },
                { id: Subject.SAXOPHONE, label: 'Saxophone', icon: '🎷', color: 'bg-amber-100 text-amber-700' },
                { id: Subject.FLUTE, label: 'Flute', icon: '🎼', color: 'bg-cyan-100 text-cyan-700' },
                { id: Subject.BASS_GUITAR, label: 'Bass Guitar', icon: '🎸', color: 'bg-slate-200 text-slate-700' },
                { id: Subject.CELLO, label: 'Cello', icon: '🎻', color: 'bg-orange-100 text-orange-700' },
                { id: Subject.TROMBONE, label: 'Trombone', icon: '🎺', color: 'bg-lime-100 text-lime-700' },
                { id: Subject.ORGAN, label: 'Organ', icon: '🎹', color: 'bg-indigo-100 text-indigo-700' },
                { id: Subject.UKULELE, label: 'Ukulele', icon: '🪕', color: 'bg-teal-100 text-teal-700' },
                { id: Subject.CLARINET, label: 'Clarinet', icon: '🎶', color: 'bg-purple-100 text-purple-700' },
                { id: Subject.HARMONICA, label: 'Harmonica', icon: '🎵', color: 'bg-rose-100 text-rose-700' },
                { id: Subject.VIOLA, label: 'Viola', icon: '🎻', color: 'bg-fuchsia-100 text-fuchsia-700' },
                { id: Subject.OBOE, label: 'Oboe', icon: '🪈', color: 'bg-green-100 text-green-700' },
                { id: Subject.FRENCH_HORN, label: 'French Horn', icon: '🎺', color: 'bg-blue-100 text-blue-700' },
                { id: Subject.BANJO, label: 'Banjo', icon: '🪕', color: 'bg-stone-200 text-stone-700' },
                { id: Subject.ACCORDION, label: 'Accordion', icon: '🪗', color: 'bg-gray-100 text-gray-700' },
            ]
        },
        {
        ...
        <section id="courses" className="py-12 md:py-16 px-4 max-w-6xl mx-auto space-y-8">
            ...
            <div className="bg-brand-blue/5 p-6 rounded-3xl border border-brand-blue/10 flex flex-col md:flex-row items-center gap-6">
                <div className="flex-1 space-y-2 text-center md:text-left">
                    ...
                    <p className="text-sm text-brand-dark/60">Akshara LearnQuest is updated daily with MOE and IGCSE trends.</p>
                </div>
            </div>
        </section>
    );
