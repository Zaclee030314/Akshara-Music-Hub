            ...
            <div className={`grid gap-6 ${single ? 'md:grid-cols-1' : 'md:grid-cols-2'}`}>
                ...
                {season && (
                    <div className="relative overflow-hidden rounded-3xl border-2 border-brand-orange/20 bg-gradient-to-br from-brand-orange/5 via-yellow-50/60 to-white p-8 shadow-xl flex flex-col">
                        ...
                        <div className="relative z-10 flex flex-col h-full">
                            <span className="text-[11px] font-black text-brand-orange uppercase tracking-widest mb-2">🏆 Season Competition</span>
                            ...
                            <div className="mt-5 space-y-2 flex-1">
                                <div className="flex items-start gap-2">
                                    ...
                                    <div>
                                        <p className="font-bold text-brand-dark">1st Prize: {season.prizeTitle}</p>
                                        {season.prizeDetails && (
                            ...
                            >
                                Join the Competition <ArrowRight size={18} className="ml-2" />
                            </Button>
                        </div>
                    </div>
                )}
                ...
                {lastFinalized && (
                    <div className="relative overflow-hidden rounded-3xl border-2 border-brand-dark/5 bg-white p-8 shadow-xl flex flex-col">
                        <span className="text-[11px] font-black text-brand-dark/40 uppercase tracking-widest mb-2">Last Season's Champions</span>
                        ...
                        <div className="mt-5 space-y-3 flex-1">
                            {lastFinalized.winners.length === 0 ? (
                                <p className="text-sm text-brand-dark/40 font-bold italic">No winners recorded.</p>
                            ) : (
                                lastFinalized.winners.map(w => (
                                    ...
                                    >
                                        ...
                                        <div className="min-w-0 flex-1">
                                            ...
                                            {w.rank === 1 && (
                                                <p className="text-xs text-brand-orange font-bold">Won: {w.prizeTitle}</p>
                                            )}
                                        </div>
                                        <span className="text-sm font-bold text-brand-dark/50 shrink-0">{w.points} pts</span>
                                    </div>
                                ))
                            )}
                        </div>
