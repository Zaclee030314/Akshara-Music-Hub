...
export const BillingPage: React.FC = () => {
    ...
    const planLabel = (level?: string | null): string => {
        if (level === 'all') return t('billing.planAll');
        if (level === 'single') return t('billing.planSingle');
    ...
    if (error || !status) {
        return <div className="text-center p-16 text-red-500 font-bold">{error || t('billing.unavailable')}</div>;
    }
    ...
    return (
        <div className="max-w-2xl mx-auto space-y-8 pt-8 pb-16 animate-in fade-in duration-500 px-4">
            <div className="text-center space-y-2">
                ...
                <h2 className="text-3xl md:text-4xl font-display font-bold text-brand-dark">{t('nav.mySubscription')}</h2>
                <p className="text-brand-dark/60">{t('billing.manageDesc')}</p>
            </div>
            ...
            {referralCreditCents > 0 && (
                <div className="flex items-start gap-2 bg-brand-orange/5 border border-brand-orange/20 text-brand-dark/70 p-4 rounded-2xl text-sm font-medium">
                    ...
                    <span>{t('billing.referralCredit', { amount: (referralCreditCents / 100).toFixed(2) })}</span>
                </div>
            )}
            ...
            <Card className="p-6 md:p-8 shadow-xl space-y-6">
                <div className="flex items-center justify-between">
                    <div>
                        <p className="text-xs font-bold text-brand-dark/40 uppercase tracking-widest">{t('billing.currentPlan')}</p>
                        <p className="text-2xl font-display font-bold text-brand-dark">{subscribed ? planLabel(status.subscriptionLevel) : t('billing.free')}</p>
                    </div>
                    {subscribed ? (
                        <span className={`text-xs font-bold px-3 py-1 rounded-full ${cancelPending ? 'bg-amber-100 text-amber-700' : 'bg-brand-green/10 text-brand-green'}`}>
                            {cancelPending ? t('billing.cancelsAtEnd') : t('billing.active')}
                        </span>
                    ) : (
                        <span className="text-xs font-bold px-3 py-1 rounded-full bg-gray-100 text-gray-500">{t('billing.free')}</span>
                    )}
                </div>
                ...
                {subscribed ? (
                    <>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm border-t border-brand-dark/5 pt-6">
                            {status.subscriptionLevel === 'single' && (
                                <div>
                                    <p className="text-xs font-bold text-brand-dark/40 uppercase">{t('billing.subscribedSyllabus')}</p>
                                    <p className="font-medium text-brand-dark">{status.subscribedSyllabus || '—'}</p>
                                </div>
                            )}
                            <div>
                                <p className="text-xs font-bold text-brand-dark/40 uppercase">{t('billing.startDate')}</p>
                                <p className="font-medium text-brand-dark">{fmtDate(status.subscriptionStartDate)}</p>
                            </div>
                            <div>
                                <p className="text-xs font-bold text-brand-dark/40 uppercase">{t('billing.renewsEnds')}</p>
                            ...
                            <div>
                                <p className="text-xs font-bold text-brand-dark/40 uppercase flex items-center gap-1">
                                    <CalendarClock size={12} /> {t('billing.daysRemaining')}
                                </p>
                                <p className="font-medium text-brand-dark">{daysLeft !== null ? t('billing.days', { count: daysLeft }) : '—'}</p>
                            </div>
                        </div>
                        ...
                        <div className="flex items-start gap-2 bg-brand-dark/5 border border-brand-dark/10 text-brand-dark/60 p-4 rounded-2xl text-xs font-medium">
                            ...
                            <span>{t('billing.passNote', { date: fmtDate(status.subscriptionEndDate) })}</span>
                        </div>
                        ...
                        {cancelPending ? (
                            <div className="space-y-4">
                                <div className="flex items-start gap-3 bg-amber-50 border border-amber-200 text-amber-800 p-4 rounded-2xl text-sm font-medium">
                                    <span>{t('billing.cancelScheduled', { date: fmtDate(status.subscriptionEndDate) })}</span>
                                </div>
                                <Button onClick={handleReactivate} disabled={working} fullWidth className="bg-brand-green hover:bg-green-600">
                                    {working ? <Loader2 className="animate-spin" size={18} /> : <><RefreshCw size={16} /> {t('billing.reactivate')}</>}
                                </Button>
                            </div>
                        ) : (
                            <Button onClick={handleCancel} disabled={working} variant="outline" fullWidth className="text-red-500 border-red-200 hover:bg-red-50">
                                {working ? <Loader2 className="animate-spin" size={18} /> : <><ShieldOff size={16} /> {t('billing.cancel')}</>}
                            </Button>
                        )}
                    </>
                ) : (
                    <div className="space-y-4 border-t border-brand-dark/5 pt-6">
                        <ul className="space-y-2 text-sm text-brand-dark/60">
                            <li className="flex items-center gap-2"><CheckCircle2 size={16} className="text-brand-green" /> {t('pricing.freeQuizzes')}</li>
                        </ul>
