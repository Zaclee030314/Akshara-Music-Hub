...
const makeWesternInstrumentTopics = (name, t) => ({
    'Grade 1': [
...
const OTHER_WESTERN_INSTRUMENTS = {
    'Trumpet': {
...
export const getInstrumentFacts = (subject) => INSTRUMENT_FACTS[subject] ?? [];
...
const makeHindustaniMelodicTopics = (name, t) => ({
    'Grade 1': [
...
// ─── Lookup ─────────────────────────────────────────────────────────────────────
export const CARNATIC_MUSIC_TOPICS = {
...
export const HINDUSTANI_MUSIC_TOPICS = {
    'Tabla': TABLA_HINDUSTANI,
    ...Object.fromEntries(Object.entries(HINDUSTANI_INSTRUMENTS).map(([name, t]) => [name, makeHindustaniMelodicTopics(name, t)]))
};
export const WESTERN_MUSIC_TOPICS = {
    ...
    'Drums': DRUMS,
    ...Object.fromEntries(Object.entries(OTHER_WESTERN_INSTRUMENTS).map(([name, t]) => [name, makeWesternInstrumentTopics(name, t)]))
};
/** Curated topic list for a music subject/grade, or null when none exists (→ AI fallback). */
export const getCuratedTopics = (syllabus, subject, grade) => {
    const map = (syllabus === 'Carnatic Music' || syllabus === 'Indian Music' /* legacy */) ? CARNATIC_MUSIC_TOPICS :
