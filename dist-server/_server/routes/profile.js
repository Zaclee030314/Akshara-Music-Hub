...
router.get('/referral-code', authenticateToken, requireParentSession, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ error: 'Unauthorized' });
...
router.post('/season-seen', authenticateToken, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ error: 'Unauthorized' });
...
router.put('/family', authenticateToken, requireParentSession, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ error: 'Unauthorized' });
        ...
        if (children !== undefined) {
            ...
            for (const c of children) {
                ...
                if (!name)
                    return res.status(400).json({ error: 'Each child needs a name.' });
        ...
        if (finalName && finalPhone && hasChild)
...
router.get('/referral-stats', authenticateToken, requireParentSession, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ error: 'Unauthorized' });
