...
const readChildInput = (raw, requireBirthday) => {
    ...
    if (raw?.birthday !== undefined && raw?.birthday !== null && raw?.birthday !== '') {
        ...
        if (!birthday)
            return { ok: false, error: `Invalid date of birth for ${name}.` };
    }
    ...
    if (raw?.gradeSyllabus) {
        if (!isValidSyllabus(raw.gradeSyllabus))
            return { ok: false, error: `Unknown syllabus for ${name}.` };
...
const shapeChild = async (c, index, seats) => {
...
const familyRootFor = async (userId) => {
...
router.post('/setup', authenticateToken, requireParentSession, async (req, res) => {
    try {
        const current = await prisma.user.findUnique({ where: { id: req.user.id } });
        if (!current)
            return res.status(404).json({ error: 'User not found' });
        if (current.parentId)
            return res.status(400).json({ error: 'This is already a child profile.' });
        if (current.role === 'parent')
            return res.status(400).json({ error: 'Family profiles are already set up.' });
        if (current.role !== 'student')
            return res.status(400).json({ error: 'Only student accounts can be converted into family accounts.' });
...
const requireParentAccount = async (req, res, next) => {
    const me = await prisma.user.findUnique({ where: { id: req.user.id }, select: { role: true } });
    if (!me || me.role !== 'parent')
        return res.status(403).json({ error: 'Family profiles are not set up on this account.' });
    next();
};
// POST /api/family/children
router.post('/children', authenticateToken, requireParentSession, requireParentAccount, async (req, res) => {
    try {
...
router.put('/children/:id', authenticateToken, requireParentSession, requireParentAccount, async (req, res) => {
    try {
        const parentId = req.user.id;
        ...
        if (req.body?.birthday !== undefined && req.body.birthday !== '' && req.body.birthday !== null) {
            if (child.birthday)
                return res.status(400).json({ error: 'Date of birth cannot be changed once set.' });
...
router.delete('/children/:id', authenticateToken, requireParentSession, requireParentAccount, async (req, res) => {
    try {
        const parentId = req.user.id;
        const child = await prisma.user.findFirst({ where: { id: req.params.id, parentId, archivedAt: null } });
        if (!child)
        ...
        const count = await prisma.user.count({ where: { parentId, archivedAt: null } });
        if (count <= 1)
            return res.status(400).json({ error: 'A family needs at least one profile.' });
...
router.post('/switch/:id', authenticateToken, requireParentSession, requireParentAccount, async (req, res) => {
    try {
        const parentId = req.user.id;
