...
const tallyVotes = (votes, optionCount) => {
...
router.post('/:id/vote', authenticateToken, async (req, res) => {
    try {
        ...
        const resolvedIndex = await prisma.$transaction(async (tx) => {
            ...
            if (!fresh)
                throw new Error('Poll disappeared mid-transaction');
...
router.put('/admin/:id', authenticateToken, requireAdmin, async (req, res) => {
    try {
        ...
        if (allowSuggestions !== undefined)
            data.allowSuggestions = !!allowSuggestions;
        if (seasonId !== undefined)
            data.seasonId = seasonId ? String(seasonId) : null;
...
const buildAdminOptions = (poll, optionCounts) => {
    const options = parseOptions(poll.options);
    const meta = parseSuggestedMeta(poll.suggestedMeta);
    const removed = new Set(parseRemoved(poll.removedOptions));
