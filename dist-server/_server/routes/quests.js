...
router.post('/', authenticateToken, checkExpiredSubscriptions, async (req, res) => {
...
router.delete('/:id', authenticateToken, checkExpiredSubscriptions, async (req, res) => {
    const { id } = req.params;
    const userId = req.user?.id;
    const userRole = req.user?.role;
    if (userRole !== 'teacher' && userRole !== 'TEACHER') {
    ...
    try {
        ...
        if (quest.creatorId !== userId)
            return res.status(403).json({ error: 'You can only delete your own quests' });
