...
const nonNegInt = (v) => Math.max(0, parseInt(String(v ?? 0), 10) || 0);
...
const hydrateUsers = async (rows, includeGrade = false) => {
...
const hydrateWinners = async (seasonId) => {
...
router.get('/history', async (_req, res) => {
    try {
        ...
        const seasons = await Promise.all(finalized.map(async (s) => ({
                ...
                if (userId) {
                    ...
                    if (mine) {
                        if (!liveAvatar.has(mine.userId)) {
                            ...
                            if (self)
                                liveAvatar.set(mine.userId, self.avatar);
                        }
                        me = toRow(mine);
                    }
                }
...
router.get('/admin/all', authenticateToken, requireAdmin, async (_req, res) => {
    try {
        const now = new Date();
        const all = await prisma.season.findMany({ orderBy: { startDate: 'desc' } });
        const seasons = await Promise.all(all.map(async (s) => ({
            id: s.id,
            name: s.name,
            description: s.description,
            prizeTitle: s.prizeTitle,
...
router.post('/admin', authenticateToken, requireAdmin, async (req, res) => {
    try {
        const { name, description, prizeTitle, prizeDetails, secondPlacePoints, thirdPlacePoints, secondPrizeTitle, thirdPrizeTitle, firstPrizeCoins, secondPrizeCoins, thirdPrizeCoins, startDate, endDate, } = req.body;
...
router.put('/admin/:id', authenticateToken, requireAdmin, async (req, res) => {
    try {
        ...
        const sameMinute = (a, b) => Math.floor(a.getTime() / 60000) === Math.floor(b.getTime() / 60000);
        ...
        const prizeValuesChanged = prizeNumbers.some(([incoming, current]) => incoming !== undefined && nonNegInt(incoming) !== current);
        if (status === 'ended') {
        ...
        if (prizeTitle !== undefined)
            data.prizeTitle = String(prizeTitle).trim();
        if (prizeDetails !== undefined)
            data.prizeDetails = prizeDetails ? String(prizeDetails) : null;
        if (secondPlacePoints !== undefined)
            data.secondPlacePoints = nonNegInt(secondPlacePoints);
        if (thirdPlacePoints !== undefined)
            data.thirdPlacePoints = nonNegInt(thirdPlacePoints);
        if (secondPrizeTitle !== undefined)
            data.secondPrizeTitle = secondPrizeTitle ? String(secondPrizeTitle).trim() : null;
        if (thirdPrizeTitle !== undefined)
            data.thirdPrizeTitle = thirdPrizeTitle ? String(thirdPrizeTitle).trim() : null;
        if (firstPrizeCoins !== undefined)
            data.firstPrizeCoins = nonNegInt(firstPrizeCoins);
        if (secondPrizeCoins !== undefined)
            data.secondPrizeCoins = nonNegInt(secondPrizeCoins);
        if (thirdPrizeCoins !== undefined)
            data.thirdPrizeCoins = nonNegInt(thirdPrizeCoins);
