...
export const isTieredEnabled = async () => (await getSetting('referral.tiered', 'true')) === 'true';
...
export const tierFor = (tiers, tiered, paidCount) => {
...
export const releaseDueInstalments = async (userId) => {
    try {
        ...
        if (total > 0)
            console.log(`[REFERRAL] Released RM${(total / 100).toFixed(2)} of due instalments to ${userId}`);
        return total;
    }
    catch (e) {
...
export const grantReferralEarning = async (referrerId, referredUserId) => {
 ...
 */
export const settleReferralGrant = async (payerId) => {
    try {
        const payer = await prisma.user.findUnique({
            where: { id: payerId },
            select: { referredById: true, referralRewardGranted: true },
        });
        if (!payer?.referredById || payer.referralRewardGranted)
            return;
...
export const referralStatsFor = async (userId) => {
    ...
    for (const e of earnings) {
        ...
        for (const i of e.instalments) {
            if (i.creditedAt)
                creditedCents += i.amountCents;
            else {
                pendingCents += i.amountCents;
                if (!nextDue || i.dueDate < nextDue)
                    nextDue = i.dueDate;
            }
        }
    }
